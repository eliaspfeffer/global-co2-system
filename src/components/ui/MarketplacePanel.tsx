...
const MarketplacePanel: React.FC = () => {
  ...
  const marketOffers = useAppStore((state) => state.marketOffers);
  const selectedCountryId = useAppStore((state) => state.selectedCountryId);
  const postOffer = useAppStore((state) => state.postOffer);
  const removeOffer = useAppStore((state) => state.removeOffer);
  const acceptTrade = useAppStore((state) => state.acceptTrade);
          ...
          <div className="p-3.5 bg-slate-600 rounded-lg space-y-2 shadow">
            <p className="text-sm text-emerald-200 text-center">
              ...
              <span className="font-bold">
                {selectedCountry.co2ForSale.amount} tons
              </span>{" "}
              at{" "}
              <span className="font-bold">
                {selectedCountry.co2ForSale.pricePerTon} BTC units/ton
              </span>
              .
            </p>
            <button
              ...
              <div>
                ...
                <p className="text-sm text-slate-200">
                  ...
                  <span className="font-bold text-white">
                    {offer.amount.toLocaleString()} tons
                  </span>
                </p>
                <p className="text-sm text-slate-200">
                  Price:{" "}
                  <span className="font-bold text-yellow-400">
                    {offer.pricePerTon.toLocaleString()} BTC units/ton
                  </span>
                </p>
              </div>
