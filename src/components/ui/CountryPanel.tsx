...
const CountryPanel: React.FC = () => {
  ...
  return (
    <div className={`p-5 rounded-xl shadow-lg text-white mt-3 ${panelBgClass} transition-colors duration-300`}>
      <h2 className={`text-2xl font-bold mb-4 pb-3 border-b-2 ${isPenalized ? 'border-purple-400/50 text-purple-200' : isInDeficit ? 'border-red-400/50 text-red-200' : 'border-sky-500/50 text-sky-300'}`}>
        {name} {isPenalized ? "(Penalized)" : ""}
      </h2>

      <div className="space-y-3 text-sm"> {/* Slightly smaller base text for details */}
        ...
        <div className="flex justify-between items-center">
          ...
          <span className={`font-mono font-medium ${isInDeficit && !isPenalized ? 'text-red-300' : (isPenalized ? 'text-purple-300' : 'text-green-300')}`}>
        ...
        <div className={`pt-4 mt-4 border-t-2 ${isInDeficit && !isPenalized ? 'border-red-400/50' : (isPenalized ? 'border-purple-400/50' : 'border-slate-500/50')}`}>
          <h3 className={`text-lg font-semibold mb-3 ${isInDeficit && !isPenalized ? 'text-red-200' : (isPenalized ? 'text-purple-200' : 'text-slate-200')}`}>
            Conceptual Deposits {isInDeficit && !isPenalized ? <span className="text-xs">(At Risk)</span> : ""}
          </h3>
          <div className="flex justify-between items-center mb-1">
            <div className="flex items-center">
              <span className="text-2xl mr-2 text-yellow-400">₿</span> {/* Bitcoin Icon */}
              <span className="font-semibold text-slate-200">Bitcoin:</span>
            </div>
            <span className={`font-mono ${isPenalized && conceptualBitcoinDeposit <=0 ? 'line-through text-slate-400' : 'text-slate-100'}`}>{conceptualBitcoinDeposit.toLocaleString()}</span>
          </div>
          <div className="flex justify-between items-center">
             <div className="flex items-center">
              <span className="text-2xl mr-2 text-amber-400">♛</span> {/* Gold Icon */}
              <span className="font-semibold text-slate-200">Gold:</span>
            </div>
            <span className={`font-mono ${isPenalized && conceptualGoldDeposit <=0 ? 'line-through text-slate-400' : 'text-slate-100'}`}>{conceptualGoldDeposit.toLocaleString()} oz</span>
          </div>
        </div>
      </div>

      {isInDeficit && !isPenalized && (
        <div className="mt-6 space-y-2.5"> {/* Increased spacing */}
          <button
