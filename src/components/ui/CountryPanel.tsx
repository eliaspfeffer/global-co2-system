...
const CountryPanel: React.FC = () => {
  ...
  const isVoteActiveForSelectedCountry = activeVoteProposals.some(
    (p) => p.targetCountryId === countryId && p.status === "active"
  );
    ...
    >
      <h2
        className={`text-2xl font-bold mb-4 pb-3 border-b-2 ${
          isPenalized
            ...
            : isVoteActiveForSelectedCountry
      ...
      <div className="space-y-3 text-sm">
        <div className="flex justify-between items-center">
          ...
          <span className="font-mono text-slate-100">
            {initialCo2Allowance.toLocaleString("en-US")} tons
          </span>
        </div>
        <div className="flex justify-between items-center">
          ...
          >
            {currentCo2Emissions.toLocaleString("en-US")} tons
          </span>
        </div>
          ...
          <div className="p-3 bg-red-500/40 rounded-md my-2 text-red-100">
            <div className="flex justify-between items-center">
              ...
              <span className="font-bold font-mono">
                {deficit.toLocaleString("en-US")} tons
              </span>
            </div>
            <p className="text-xs mt-1">
              This country needs to acquire {deficit.toLocaleString("en-US")}{" "}
              tons of CO2 allowances or face penalties.
            </p>
          </div>
        )}
        {isVoteActiveForSelectedCountry && !isPenalized && (
          <div className="p-3 bg-orange-500/40 rounded-md my-2 text-orange-100">
            <p className="text-base font-semibold text-center">
              A penalty vote is currently active against {name}!
            </p>
            <p className="text-xs mt-1 text-center">
              Other nations are deciding on potential sanctions.
            </p>
          </div>
        )}
        {!isInDeficit && deficit < 0 && !isPenalized && (
          <div className="p-3 bg-green-500/40 rounded-md my-2 text-green-100">
            <div className="flex justify-between items-center">
              ...
              <span className="font-bold font-mono">
                {Math.abs(deficit).toLocaleString("en-US")} tons
              </span>
            </div>
            <p className="text-xs mt-1">
        ...
        >
          ...
          <div className="flex justify-between items-center mb-1">
            ...
            <span
              className={`font-mono ${
                (isPenalized ||
                  (isVoteActiveForSelectedCountry && isInDeficit)) &&
                conceptualBitcoinDeposit <= 0
                  ? "line-through text-slate-400"
                  : "text-slate-100"
              }`}
            >
              {conceptualBitcoinDeposit.toLocaleString("en-US")}
            </span>
          </div>
          <div className="flex justify-between items-center">
            <div className="flex items-center">
            ...
            <span
              className={`font-mono ${
                (isPenalized ||
                  (isVoteActiveForSelectedCountry && isInDeficit)) &&
                conceptualGoldDeposit <= 0
                  ? "line-through text-slate-400"
                  : "text-slate-100"
              }`}
            >
              {conceptualGoldDeposit.toLocaleString("en-US")} oz
            </span>
          </div>
        </div>
      </div>
      ...
      {isInDeficit && !isPenalized && !isVoteActiveForSelectedCountry && (
        <div className="mt-6 space-y-2.5">
          ...
          <div className="p-2 bg-red-800/50 rounded-md ring-2 ring-red-500 animate-pulse">
            {" "}
            {/* Pulsing warning for this button */}
            <button
            ...
            <p className="text-xs text-red-200 mt-1.5 text-center">
              Warning: This may lead to other nations voting on penalties
              against your country's deposits.
            </p>
          </div>
        </div>
      )}
      {!isInDeficit &&
        deficit < 0 &&
        !isPenalized &&
        !isVoteActiveForSelectedCountry && ( // Hide if vote active for them
          <div className="mt-6">
