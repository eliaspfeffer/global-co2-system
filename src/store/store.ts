...
const deepCopyAndPrepareInitialCountries = (): Country[] => {
  return (JSON.parse(JSON.stringify(initialCountriesData)) as Omit<Country, 'originalInitialAllowance'>[]).map(country => ({
    ...country,
    isPenalized: false,
...
const calculateCurrentSumOfAllowances = (countries: Country[]): number => {
  return countries.reduce((sum, country) => sum + country.initialCo2Allowance, 0);
};
...
export const selectCurrentGlobalEmissions = (state: AppState) =>
  state.countries.reduce((sum, country) => sum + country.currentCo2Emissions, 0);
...
export const useAppStore = create<AppState>((set, get) => {
  ...
  const initialGlobalBudgetValue = calculateCurrentSumOfAllowances(initialCountries);
  ...
  return {
    ...
    adjustEmissions: (countryId, newEmissionLevel) => {
        set((state) => ({
            ...
            c.id === countryId ? { ...c, currentCo2Emissions: Math.max(0, newEmissionLevel) } : c // Ensure non-negative
            ),
        }));
    },
    ...
    postOffer: (sellerId, amount, pricePerTon) => {
        ...
        if (!seller) { console.error("Seller not found for posting offer."); return; }
        const surplus = seller.initialCo2Allowance - seller.currentCo2Emissions;
        if (amount <= 0 || pricePerTon <= 0) { alert("Offer amount and price must be positive."); return; }
        if (amount > surplus) { alert(`Cannot offer more than available surplus (${surplus.toLocaleString()} tons).`); return; }
    ...
    acceptTrade: (buyerId, acceptedOffer) => {
        ...
        const seller = get().countries.find(c => c.id === acceptedOffer.sellerId);

        if (!buyer || !seller) { console.error("Buyer or Seller not found for trade."); return; }

        const buyerDeficit = buyer.currentCo2Emissions - buyer.initialCo2Allowance;
        if (buyerDeficit <= 0) { alert(`${buyer.name} has no deficit to cover.`); return; }

        const amountToTrade = Math.min(acceptedOffer.amount, buyerDeficit);
        if (amountToTrade <= 0) { alert("No amount to trade."); return;} // Should not happen if deficit > 0

        const totalCost = amountToTrade * acceptedOffer.pricePerTon;
        if (buyer.conceptualBitcoinDeposit < totalCost) { alert(`${buyer.name} cannot afford this trade (Cost: ${totalCost}, Available: ${buyer.conceptualBitcoinDeposit}).`); return; }
        ...
        set((state) => {
            const updatedCountries = state.countries.map(c => {
                ...
                if (c.id === acceptedOffer.sellerId) {
                    const newSellerCo2ForSale = c.co2ForSale && (c.co2ForSale.amount - amountToTrade > 0)
                                            ? { ...c.co2ForSale, amount: c.co2ForSale.amount - amountToTrade }
                                            : undefined;
                    return { ...c, initialCo2Allowance: c.initialCo2Allowance - amountToTrade, conceptualBitcoinDeposit: c.conceptualBitcoinDeposit + totalCost, co2ForSale: newSellerCo2ForSale };
                }
                return c;
            });
            const updatedMarketOffers = state.marketOffers.map(offer =>
                offer.sellerId === acceptedOffer.sellerId
                ? { ...offer, amount: offer.amount - amountToTrade }
                : offer
            ).filter(offer => offer.amount > 0);

            const newGlobalBudget = calculateCurrentSumOfAllowances(updatedCountries);
            return { countries: updatedCountries, marketOffers: updatedMarketOffers, currentGlobalBudget: newGlobalBudget };
        });
    },

    initiatePenaltyVote: (targetCountryId) => {
        const targetCountry = get().countries.find(c => c.id === targetCountryId);
        if (!targetCountry) { console.error("Target country for vote not found."); return; }
        if (get().activeVoteProposals.some(p => p.targetCountryId === targetCountryId && p.status === 'active')) { alert(`A penalty vote for ${targetCountry.name} is already active.`); return; }
        if (targetCountry.currentCo2Emissions <= targetCountry.initialCo2Allowance) { alert(`${targetCountry.name} is not in deficit, no penalty vote needed.`); return; }
    ...
    castVote: (voterCountryId, targetCountryId, voteChoice) => {
        set(state => {
            ...
            if (proposalIndex === -1) { console.warn("Vote is closed or proposal not found."); return state; }
            ...
            if (proposal.votes[voterCountryId]) { alert("You have already voted on this proposal."); return state; }
            ...
            if (yesVotes / eligibleVoters >= MAJORITY_THRESHOLD) {
                ...
                if (target) {
                    const btcPenalty = Math.floor(target.conceptualBitcoinDeposit * 0.5); // Use floor for whole numbers
                    const goldPenalty = Math.floor(target.conceptualGoldDeposit * 0.5);

                    newSatelliteFund.bitcoin += Math.floor(btcPenalty * SATELLITE_FUND_PERCENTAGE);
                    newSatelliteFund.gold += Math.floor(goldPenalty * SATELLITE_FUND_PERCENTAGE);

                    newCountriesState = newCountriesState.map(c =>
                        c.id === targetCountryId ? {
                            ...c,
                            conceptualBitcoinDeposit: target.conceptualBitcoinDeposit - btcPenalty,
                            conceptualGoldDeposit: target.conceptualGoldDeposit - goldPenalty,
                            isPenalized: true
                        } : c);
                    updatedProposal.status = 'executed'; // Final status
                    alert(`Penalty executed against ${target.name}! Deposits reduced by 50%. Satellite fund credited.`);
                }
            } else if (totalVotesCast === eligibleVoters) { // All voted, but majority not reached
                updatedProposal.status = 'failed';
                updatedProposal.voteTalliedAt = Date.now();
                const targetName = get().countries.find(c => c.id === targetCountryId)?.name || 'Unknown';
                alert(`Penalty vote against ${targetName} failed. Majority not reached.`);
            }

            const newProposals = [...state.activeVoteProposals];
            newProposals[proposalIndex] = updatedProposal;
            return { ...state, countries: newCountriesState, activeVoteProposals: newProposals, satelliteNetworkFund: newSatelliteFund };
        });
    },
    ...
    fastForwardTime: () => {
        set(state => {
            const updatedCountries = state.countries.map(country => {
                ...
                const emissionIncrease = Math.round(country.originalInitialAllowance * 0.02);
                let newEmissions = country.currentCo2Emissions + emissionIncrease;
                // Cap emissions at 3x original allowance
                newEmissions = Math.min(newEmissions, country.originalInitialAllowance * 3);

                const minAllowance = Math.round(country.originalInitialAllowance * MIN_ALLOWANCE_FLOOR_PERCENTAGE_OF_ORIGINAL);
                let newAllowance = Math.round(country.initialCo2Allowance * (1 - ANNUAL_ALLOWANCE_REDUCTION_PERCENTAGE));
                newAllowance = Math.max(newAllowance, minAllowance);

                return { ...country, currentCo2Emissions: newEmissions, initialCo2Allowance: newAllowance };
            });
    ...
    resetScenario: () => {
        ...
        const freshInitialGlobalBudget = calculateCurrentSumOfAllowances(freshCountries);
        set({
